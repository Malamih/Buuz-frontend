...
export const Content = () => {
  ...
  return (
    <div className="content" style={{ gridArea: "content" }}>
      <div className="header flex items-center justify-between pr-6">
        <h1 className="text-xl font-bold">
          Total Projects: {data?.payload?.length}
        </h1>
        <div className="options">
          <Link href={"/dashboard/import"}>
            <div className="importProjects py-[.4rem] px-3 rounded-lg cursor-pointer hover:bg-primary transition duration-200 bg-darkPrimary">
              <ImportIcon />
            </div>
          </Link>
        </div>
      ...
      <div className="projects mt-4">
        {isFetching || isPending ? <Loader content="Fetching" /> : ""}
